...
- [ ] 환영 인사와 함께 상품명, 가격, 프로모션 이름, 재고를 안내한다.
  ...
  - {상품명} {가격} {개수}개 {프로모션 이름}
    ...
    ``` 
    - [ ] 가격과 개수에는 1,000단위 쉼표(`,`) 포맷을 적용한다.
- [ ] 재고가 0개인 상품의 재고는 `재고 없음`으로 출력한다.
...
- [ ] 구매할 상품과 수량을 입력 받는다.
    ...
    [콜라-10],[사이다-3]
    ```
- [ ] 입력값에 대해, 상품명과 수량 데이터를 추출한다.
- [ ] 입력값을 검증한다.
    - [ ] 상품과 수량 형식이 올바른지 확인한다.
    - [ ] 구매 수량에 숫자 외에 다른 문자가 있는지 확인한다.
    - [ ] 상품명이 존재하는 상품인지 확인한다.
